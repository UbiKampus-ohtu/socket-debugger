...
def send_random_room_motion():
  index = random.randrange(0, len(rooms))
  send_motion(rooms[index])
...
def send_string(data):
  ...
  send_bytes('192.168.0.100', 9000, result)
...
def send_reservation(roomName, reserved = True):
  asd = random.randrange(0, 10)
  send_motion(roomName)
  if asd > 5:
    send_motion(roomName)
